...
class Matern(StationaryKernel):
    ...
    def draw_fourier_samples(self, F, num_samples, lengthscale, variance):
        ...
        z = np.random.beta(a=self.input_dim/2., b=self.order+0.5,
                           size=(1, num_samples))
        r = np.sqrt(((self.order+0.5)*z)/(2*(np.pi**2)*(1-z)))
        r = mx.nd.array(r, dtype=self.dtype, ctx=self.ctx)
        unit_w = F.random.normal(shape=(self.input_dim, num_samples),
                                 dtype=self.dtype, ctx=self.ctx)
        unit_w /= F.sqrt(F.sum(F.square(unit_w), axis=0, keepdims=True))
        W = r*unit_w/F.expand_dims(lengthscale, axis=-1)*(2*np.pi)
