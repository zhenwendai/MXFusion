...
from nfw.vae_comb_encoder import VAE
...
if __name__ == '__main__':
    ...
    encoder = nn.Sequential(nn.Linear(output_dim, H),
                            nn.ReLU(),
                            nn.Linear(H, H),
                            nn.ReLU(),
                            nn.Linear(H, latent_dim)).to(device)

    vae = VAE(decoder, encoder, data.shape[-1], latent_dim, Q, conf['mlp_hidden'], conf['num_samples'],
    ...
    for i in range(conf['num_epochs']):
        ...
        for batch_idx, (data_batch,) in enumerate(train_loader):
            optimizer.zero_grad()
            if dataset_dync:
                die = torch.rand(*data_batch.shape, dtype=data_batch.dtype, device=device)
                binarized_data = die < data_batch.to(device, non_blocking=True)
            else:
    ...
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, vae.parameters()),
                                 lr=conf['lr_2nd'])
    vae.sigma2_rev.requires_grad = True
