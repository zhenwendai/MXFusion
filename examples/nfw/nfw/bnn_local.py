...
class BNN_local(nn.Module):
    def __init__(self, num_units, Q, mlp_units, num_samples, device):
        ...
        self.sigma2_trans = torch.Tensor([1e-4]).to(device)
    ...
    def forward(self, x, target):
        ...
        jacobT = jacob.transpose(-2, -1)

        jacob_sq = torch.matmul(jacobT, jacob)
        L = torch.cholesky(jacob_sq+torch.eye(jacob.shape[-1], device=self.device)[None, :, :]*self.sigma2_trans)

        logdet_r = torch.log(torch.diagonal(L, dim1=-2, dim2=-1).abs()).sum(-1)

        jz = torch.matmul(jacob, z_sample.reshape(z_sample.shape+(1,)))
        LInvJ = torch.triangular_solve(jacobT, L, upper=False)[0]

        log_q_x = -logdet_r - (self.param_size - self.Q)/2*torch.log(self.sigma2_trans) \
            - (self.param_size)/2 * np.log(2*np.pi) \
            - (jz**2).sum(-1).sum(-1)/(2*self.sigma2_trans) \
            + (torch.matmul(LInvJ, jz)**2).sum(-1).sum(-1)/(2*self.sigma2_trans)
        ...
        logL = log_p + log_p_theta - log_q_x
